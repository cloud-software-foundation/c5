 ...
 */
public class Encoder<T extends Message<T>> extends MessageToMessageEncoder<Message<T>> {
  @Override
  protected void encode(ChannelHandlerContext ctx, Message<T> msg, List<Object> out) throws Exception {
    ...
    ByteBuf result = Unpooled.wrappedBuffer(buffers.toArray(new ByteBuffer[]{}));
    out.add(result);
  }
}
