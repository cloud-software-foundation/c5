...
public class InRamLeaderTest {
  ...
  public void testCommit() throws Exception {
    ...
    repl.logData(TEST_DATUM);
    assertEventuallyIndexCommits(2);
  }
  ...
  public void testFollowerCatchup() throws Exception {
    ...
    repl.logData(TEST_DATUM);
    assertEventuallyIndexCommits(1);
    ...
    repl.logData(TEST_DATUM);
    repl.logData(TEST_DATUM);
    assertEventuallyIndexCommits(3);
  ...
  public void testFollowerWhoReceivedNothing() throws Exception {
    ...
    assertEventuallyIndexCommits(10);
   ...
   */
  private void assertEventuallyIndexCommits(long index) throws InterruptedException {
    ReplicationModule.IndexCommitNotice lastCommitNotice;
    do {
      lastCommitNotice = commits.poll(5, TimeUnit.SECONDS);
      assertThat("Timed out waiting for next IndexCommitNotice",
          lastCommitNotice, is(not(nullValue())));
    } while (lastCommitNotice.committedIndex < index);

    assertThat(lastCommitNotice.committedIndex, is(index));
    assertThat(commits.isEmpty(), is(true));
  }
  ...
  private void routeOutboundRequests(Request<RpcRequest, RpcWireReply> request) {
    long to = request.getRequest().recipientNodeId;
  ...
  private void replyAckToRequest(Request<RpcRequest, RpcWireReply> request, boolean success) {
    ...
    long to = message.recipientNodeId;
  ...
  private void sendAppendEntriesReply_Success(Request<RpcRequest, RpcWireReply> request) {
    ...
    RpcWireReply replyMessage = new RpcWireReply(incomingMessage.recipientNodeId, QUORUM_ID,
