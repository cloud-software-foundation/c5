...
public class TabletService extends AbstractService implements TabletModule {
  ...
  private static final long TIME_TO_FIRST_FLUSH = 15;
  private static final long TIME_TO_FIRST_COMPACT = 60;
    ...
    Futures.addCallback(future, new FutureCallback<ReplicationModule.Replicator>() {
      ...
      public void onSuccess(ReplicationModule.Replicator result) {
        try {
          ...
          final HRegion region = HRegion.openHRegion(new org.apache.hadoop.fs.Path(server.getConfigDirectory().baseConfigPath.toString()),
              regionInfo,
              tableDescriptor,
              shim,
              conf,
          ...
          }, TIME_TO_FIRST_FLUSH, TIME_TO_FIRST_FLUSH, TimeUnit.SECONDS);
          fiber.scheduleAtFixedRate(new Runnable() {
            @Override
            public void run() {
              try {
          ...
          }, TIME_TO_FIRST_COMPACT, TIME_TO_FIRST_COMPACT, TimeUnit.SECONDS);
